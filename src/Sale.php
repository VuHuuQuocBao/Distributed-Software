...
<?php	
    ...
    if($brand != null)
    {
        $command = "SELECT * FROM `sanpham` WhERE brand = '".$brand."' and giaGiam > 0";
    }
    else
    {
        //load theo Loại sản phẩm
        if($IDLoai != -1)
	        $command = "SELECT * FROM `sanpham` WhERE IDLoai = '".$IDLoai."' and giaGiam > 0";
        else if($IDLoai == -1) //load tất cả
            $command = "SELECT * FROM `sanpham` WhERE giaGiam > 0";
    }
